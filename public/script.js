...
async function toggleWishlist(e, productId) {
 ...
 async function toggleWishlist(e, productId) {
  ...
  const button = e.currentTarget; // pakai currentTarget supaya pasti tombol yang dipanggil

  try {
    const response = await fetch('/auth/wishlist', {  // endpoint bisa disesuaikan jika beda
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ product_id: productId }),
      credentials: 'include' // penting agar cookie JWT ikut terkirim
    });
