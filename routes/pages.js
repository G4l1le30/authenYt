...
router.get("/", async (req, res) => {
  try {
    ...
    const [topSellers] = await pool.query(`
        ...
        `)
    const [dressStyles] = await pool.query("SELECT id, name, image_url, slug FROM styles ORDER BY id")
...
router.get("/seller/:userId", async (req, res) => {
  try {
    ...
    const [sellerRows] = await pool.query("SELECT id, name, email, profile_image_url, bio FROM users WHERE id = ?", [
      userId,
    ])
    ...
    const [sellerStats] = await pool.query(
      "SELECT AVG(p.rating) as average_seller_rating, COUNT(p.id) as product_count FROM products p WHERE p.user_id = ? AND p.rating IS NOT NULL",
      [userId],
    )
...
router.get("/allProduk", async (req, res) => {
  try {
    ...
    const [productColorRows] = await pool.query(
      'SELECT DISTINCT available_colors FROM products WHERE available_colors IS NOT NULL AND available_colors != ""',
    )
    ...
    const currentSelectedColorNames = selectedColorsQuery
      ? selectedColorsQuery
          .split(",")
          .map((c) => c.trim())
          .filter((c) => c)
    ...
    if (selectedSizesQuery) {
      ...
      if (Array.isArray(selectedSizesQuery)) {
        ...
        currentSelectedSizes = selectedSizesQuery
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s)
      }
    ...
    // PERBAIKAN: Kondisi filter ukuran yang lebih robust
    ...
    if (currentSelectedColorNames.length > 0) {
      const colorNameConditions = currentSelectedColorNames.map((colorName) => {
        queryParams.push(colorName)
        return `FIND_IN_SET(?, p.available_colors)`
      })
      conditions.push(`(${colorNameConditions.join(" OR ")})`)
    }

    // Filter Harga
    const currentMinPrice = Number.parseFloat(minPriceQuery)
    const currentMaxPrice = Number.parseFloat(maxPriceQuery)
    if (!isNaN(currentMinPrice) && currentMinPrice >= 0) {
      conditions.push("p.price >= ?")
      queryParams.push(currentMinPrice)
    }
    if (
      !isNaN(currentMaxPrice) &&
      currentMaxPrice > 0 &&
      (isNaN(currentMinPrice) || currentMaxPrice >= currentMinPrice)
    ) {
      conditions.push("p.price <= ?")
    ...
    const filterSizeOptions = PREDEFINED_SIZES.map((size) => ({
      name: size,
      name_dashed: size.toLowerCase().replace(/\s+/g, "-"),
    ...
    res.render("allProduk", {
      ...
      allStyles,
      user: req.user,
      currentCategoryId: currentCategoryIdForView,
      currentCategoryName,
      currentStyleSlug: currentStyleSlugForView,
      currentStyleName,
      filterColorOptions,
      filterSizeOptions,
      currentSelectedSizes,
      currentSelectedColors: currentSelectedColorNames,
      ...
      sortOptions: SORT_OPTIONS.map((opt) => ({
        ...opt,
        isSelected: opt.value === (activeSortOption ? activeSortOption.value : SORT_OPTIONS[0].value),
      })),
      currentSortValue: activeSortOption ? activeSortOption.value : SORT_OPTIONS[0].value,
    })
  } catch (err) {
    ...
    try {
      const [fallbackCategories] = await pool.query("SELECT id, name FROM categories ORDER BY name")
      const [fallbackStyles] = await pool.query("SELECT id, name, slug FROM styles ORDER BY name")

      res.status(500).render("allProduk", {
        products: [],
        pageTitle: "Error Loading Products",
        filterDescription: "An error occurred.",
        allCategories: fallbackCategories,
        allStyles: fallbackStyles,
        user: req.user,
        filterColorOptions: PREDEFINED_COLORS_CONFIG.map((c) => ({
          ...c,
          name_dashed: c.name.toLowerCase().replace(/\s+/g, "-"),
          isActive: false,
        })),
        filterSizeOptions: PREDEFINED_SIZES.map((s) => ({
          name: s,
          name_dashed: s.toLowerCase().replace(/\s+/g, "-"),
          isActive: false,
        })),
        sortOptions: SORT_OPTIONS.map((opt) => ({ ...opt, isSelected: opt.value === SORT_OPTIONS[0].value })),
        currentSortValue: SORT_OPTIONS[0].value,
        message: { type: "danger", text: "Failed to load product data. Please try again." },
      })
    } catch (fallbackError) {
      console.error("Error in fallback query:", fallbackError)
      res.status(500).render("500", {
        user: req.user,
        message: "A critical error occurred while loading the page.",
      })
    }
  }
})
...
router.get("/sell", authMiddleware.protect, async (req, res) => {
  try {
    ...
    const previouslySelectedColors = currentValues.available_colors
      ? currentValues.available_colors.split(",").map((c) => c.trim())
      : []
    ...
    const previouslySelectedSizes = currentValues.available_sizes
      ? currentValues.available_sizes.split(",").map((s) => s.trim())
      : []
    ...
    res.status(500).render("sell", {
      ...
      user: req.user,
      currentValues: {},
      availableColorsList: PREDEFINED_COLORS_CONFIG.map((c) => ({
        ...c,
        name_dashed_lowercase: c.name.toLowerCase().replace(/\s+/g, "-"),
        isSelected: false,
      })),
      availableSizesList: PREDEFINED_SIZES.map((s) => ({
        name: s,
        name_dashed_lowercase: s.toLowerCase().replace(/\s+/g, "-"),
        isSelected: false,
      })),
...
router.get("/checkout", authMiddleware.protect, async (req, res) => {
  ...
  try {
    ...
    const itemsForView = cartItemsDetails.map((item) => {
      ...
      const subtotal =
        item.product_is_visible && item.quantity <= item.current_stock ? item.current_price * item.quantity : 0 // Hanya hitung subtotal jika valid
      grandTotal += subtotal
...
router.get("/chat/:sellerId", authMiddleware.protect, async (req, res) => {
  if (!req.user) {
    // Seharusnya sudah ditangani oleh authMiddleware.protect
    return res.redirect("/login")
  }
  ...
  try {
    ...
    const [sellerRows] = await pool.query("SELECT id, name, profile_image_url FROM users WHERE id = ?", [sellerId])
    ...
    res.render("chatPage", {
      // Nama file template baru: chatPage.hbs
